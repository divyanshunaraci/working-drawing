...
class floor_plan_outline1(object):
    ...
    def __init__(self, drawing_list, thickness):
        ...
        # this data get updated in remove duplicate function. That means finally this variable does not contend the duplicate horizontal or vertical dimension labels.
        self.all_detail = {'horizontal': self.draw_1_hor_dict,
                           'vertical': self.draw_1_ver_dict}
        # this method remove all the duplicate dimensions that are occurring twice or thrice because of the same size of components all sections along horizontal or vertical directions
        ...
        for i_list in range(len(list_keys)):
            ...
            for i_value in range(len(draw_1_hor_dict[key])):
                ...
                if len(values) == 2:  # that means nothing was assigned to it earlier
                    # we do not plot any dimension for the lines with less than two T thickness
                    if values[1]-values[0] < 2*thickness:
                        self.draw_1_hor_dict[key][i_value].append([strr])
                    else:
                        # we check to make sure that up to two T thickness nothing exist at the bottom of this interior line. If there is nothing then this check passes this means we can draw the line I mean the dimension below this interior line
    ...
    def __update_int_hor(self, i_l, list_key, x1, x2, x0, xn, yi, thickness, strr):
        ...
        for keys in list_keys:
            for i in range(len(draw_1_hor_dict[keys])):
                ...
                # if it is already tagged tagged as some other line the new need to do this operation
                if len(values) == 2:
                    # while np.sum(filling_array) <= x2-x1:
                    # this check is making sure that the gnu line for which we are finding the dimension recides inside the exterior dimension it is next to. If it is larger than the exterior dimension then we solve it some other way.
    ...
    def __create_dict(self, drawing):  # create dictionary only returns the dictionary created for horizontal and vertical lines. It removes the duplication of the lines all it takes care of the overlapping of the lines. It also provides the lines in the order of its distance from origin. It does not provide any detail about the exterior or interior type of line.
        ...
        # oh key value is created for all the horizontal and vertical lines and the key is created at the common node. It is initialized with a list . Afterwards the list will be filled with the coordinates
...
class floor_plan_additional(object):
    ...
    def __init__(self, j_object, room_names, room_view_name):
        self.j_object = j_object
        self.new_j_object = self.return_new_j_object(
            self, j_object, room_names, room_view_name)
    ...
    def return_new_j_object(self, j_object, room_names, room_view_name):
        ...
        # the thickness needs to be changed
        fp0 = floor_plan_outline1(
                    ...
                    if j_object['rooms'][key1].has_key(key2):
                        ...
                        if key2 == 'room_top_view':
                            drawing_list1 = self.output_list_room_top_views(
                                key1, key2, j_object)
                            # need changes to this value
                            fp1 = floor_plan_outline1(
                                drawing_list1, j_object['rooms'][key1][key2]['thickness'])
...
class floor_plan_component1(object):
            ...
            if len(drawing_1_list) != 0:
                ...
                if 'floor_components' in list(j_object['rooms'][room_name][view_name]):
                    if 'library' in list(j_object['rooms'][room_name][view_name]['floor_components']):
                        ...
                        for items in j_object['rooms'][room_name][view_name]['floor_components']['library']:
                            drawing_2_list = j_object['rooms'][room_name][view_name][
                                'floor_components']['library'][items]['outline']
                            drawing_2_list = self.__clean_drawing_list(
                                drawing_2_list)
    ...
    def __creating_dimensions_top_front(self, dict1, drawing_list):
        ...
        dimension_list = self.__distance_to_component(
            self, dict1, outer_dim_dict, dim_dict, dimension_list, 100)
    ...
    def __checking_outer_existence(self, outer_dim_dict, key1, yi, up_or_down, x1, x2, x0, xn, thickness=200):
        """
        ...
        if not outer_dim_dict[key1].has_key(yi):
            ...
            # it was 10 , i made it 100 that messed up - now all looks too good.
            if sum(np_array_yi[x1-x0:x2-x0]) < 5:

                outer_dim_dict[key1][yi][x1-x0:x2-x0] = 1
                return yi
            else:
                # it was 100, i made it 10 that messed up- now all looks too good.
                yi = yi + up_or_down*thickness
